    ...
    const { isDragging, previewRef, handleRef } = useDraggable({
      ...
      onDropHandler: (_, { dragItem }) => {
        try {
          ...
          const dragItemObj = dragItem as unknown;
          // Then cast to a more specific type
          const id = (dragItemObj as { element: { id: string } }).element.id;
          
          if (id) {
            // Use the BlockSelectionPlugin API to set the selection
            editor.getApi(BlockSelectionPlugin).blockSelection.set(id);
          }
        } catch (error) {
          console.error('Error in onDropHandler:', error);
        }
      },
    });
