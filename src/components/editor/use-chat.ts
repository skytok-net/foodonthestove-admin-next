...
export const useChat = () => {
  ...
  return useBaseChat({
    ...
    api: '/api/ai/command',
    body: {
      apiKey: keys.openai || process.env.NEXT_PUBLIC_OPENAI_API_KEY,
      model: model.value,
    },
    fetch: async (input, init) => {
      try {
        ...
        if (!res.ok) {
          console.warn('AI API request failed, using mock data instead');
          // Mock the API response if the API call fails
  ...
  return new ReadableStream({
    async start(controller) {
      ...
      if (streamProtocol === 'data') {
        controller.enqueue(
          encoder.encode(`d:{"finishReason":"stop","usage":{"promptTokens":0,"completionTokens":${blocks.reduce(
